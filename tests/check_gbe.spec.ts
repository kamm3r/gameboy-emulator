...
function runEmulator(romPath: string, maxCycles: number = 2_000_000): { passed: boolean; output: string; cycles: number; debugInfo: string } {
  ...
  emulation_reset();
  dbg_clear();
  const success = emulation_load(data);
  if (!success) {
    return { passed: false, output: "Failed to load ROM", cycles: 0, debugInfo: "" };
  }
  ...
  for (let frame = 0; frame < maxFrames; frame++) {
    const ctx = emulation_get_context();
    if (ctx.status === "error" || !ctx.running) {
      break;
    }
  ...
  const output = String.fromCharCode(...serialWrites.filter(c => c !== 0));
  const dbgOutput = dbg_get_messages();
  ...
  const finalOutput = output || dbgOutput;
...
for (const rom of TEST_ROMS) {
  test(rom, () => {
    ...
    expect(result.passed, `ROM ${rom} failed: ${result.debugInfo} - ${result.output}`).toBe(true);
  });
}
